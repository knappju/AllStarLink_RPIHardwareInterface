...
HALStatus_t HALLoadConfig(HAL_t *hal, const char *configFilePath) {
    ...
    if (nLeds > 0) {
        ...
        if (!hal->leds) { free(hal->buttons); hal->buttons = NULL; json_object_put(root); return HAL_ERROR_ALLOC; }
    }
    ...
    for (int i = 0; i < len; i++) {
        ...
        /* MCP23017Led: wire up wrappers here when the driver is implemented */
    }
